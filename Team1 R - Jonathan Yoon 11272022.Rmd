...
```{r drift}
ph_drift <- rwf(train_ts)
drift_pred <- forecast(ph_drift, 5)
accuracy(drift_pred$mean, valid_ts)
```
...
autoplot(train_ts) +
  autolayer(valid_ts) +
  autolayer(naive_pred$mean) +
  autolayer(mean_pred$mean) +
  autolayer(lm_pred$mean) +
  autolayer(drift_pred$mean) +
  autolayer(arima_pred$mean) +
  autolayer(ets_pred$mean)
```